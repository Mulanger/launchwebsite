...
- Leaderboard sort includes `Profit`, backed by the whale server's all-time `GET /v1/leaderboard?sort=profit` mode. Profit rows are ranked directly from resolved BUY `trade_outcomes` since tracking began, not from the active `1D`/`7D` window cohort. In the profit view, hide the day-window controls, show an `All-time P/L` label, show signed `Profit` as the primary right-side metric, show `P/L trades` from the all-time P/L trade count, and replace `Avg` with recent form from the latest 5 resolved wallet trades. The percentage displayed beside the recent-form chips is the all-time resolved win rate (`allTimeWinRatePct`), not the win rate of only those five chips.
...
- Leaderboard `Profit` sort must use `/v1/leaderboard?sort=profit` and all-time realized P/L from resolved BUY outcomes, not the selected window's volume/trade aggregate. Time-window controls must be hidden in profit mode and must not affect profit fetches, rows, ranks, or values. Do not show volume as the primary metric in profit sort; show signed profit, the all-time P/L trade count, all-time resolved win rate, and the latest 5-trade W/L form instead of average trade size.
...
5. On `/leaderboard`, choose `Sort: Profit` and verify rows show/sort by signed all-time profit, not volume; each profit row should show recent form chips from the latest 5 resolved wallet trades instead of `Avg`, with the adjacent percentage showing all-time resolved win rate. Switching `1D`/`7D` must not recompute a wallet's profit from that window.
