...
- `/sitemap.xml` is generated by Next and includes only qualified market pages: at least 3 tracked whale trades and at least $50K tracked whale volume.
...
D:\polywatch-website
  ...
  app/                     Next App Router public routes and legacy app route wrappers.
    sitemap.js             Dynamic sitemap with qualified market pages.
...
Market pages are not hybrid legacy wrappers. `/market/[slug]` is a native Next server page. It prefers the API server's enriched `/v1/market-pages/:slug` response, which is populated by the watcher `market_page_snapshots` worker. Unknown market slugs return 404. Known but weak markets render with `noindex,follow`; qualified markets become indexable and are included in the dynamic sitemap. The old whale-feed scan remains as fallback for safe deployment order.
...
9. For SEO route changes, verify source HTML and JSON-LD for affected native pages such as `/market/[slug]`, `/trader/[wallet]`, `/qa`, `/qa/[slug]`, `/compare`, and `/compare/polymarket-vs-kalshi`.
10. Verify `/sitemap.xml` plus the dedicated `/sitemap-qa.xml`, `/sitemap-traders.xml`, and `/sitemap-compare.xml` routes when changing those page families.
11. For Next branch routing changes, start local Next (`npm run next:start -- -p 3000`) and verify direct route refreshes, page source HTML, hydrated UI, `/api/*`, websocket connection, follows, alerts, and mobile views.
...
- Live `robots.txt` lists `sitemap.xml`, `sitemap-qa.xml`, `sitemap-traders.xml`, and `sitemap-compare.xml`.
